...
def main(args):
    ...
    # configure lora
    if args.lora_ckpt_uuid:
        ckpt_path = os.path.join('src/smc_lora_ft/checkpoints', args.lora_ckpt_uuid)
    ...
    elif args.reward_name == "hps":
        reward_fn = rewards.hps_score(device = 'cuda')
    else:
    ...
    for prompt_idx, item in enumerate(tqdm(prompt_data)):
        if prompt_idx < args.start_from_prompt_idx:
            continue
        prompt = item["prompt"]
...
def get_args():
    ...
    parser.add_argument("--lora_ckpt_uuid", type=str, default="")
    parser.add_argument("--start_from_prompt_idx", type=int, default=0)
